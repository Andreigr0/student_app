...
class InviteStatus(PyEnum):
    ...
    def label(self):
        return {
            InviteStatus.New: 'Новое',
            InviteStatus.Revoked: 'Отозвано',
            InviteStatus.InTeam: 'В команде',
            InviteStatus.Refusal: 'Отказ компании',
            InviteStatus.Viewed: 'Просмотрено',
            InviteStatus.Consent: 'Согласие',
            InviteStatus.Rejection: 'Отказ студента',
            InviteStatus.Expired: 'Истёк срок',
        }[self]

    @staticmethod
    def map():
        return {status.value: status.label() for status in InviteStatus}

    @staticmethod
    def values():
        return [status.value for status in InviteStatus]
...
class StudentReport(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    periodId = Column(Integer, ForeignKey("project_report_periods.id"), nullable=False, index=True)
    memberId = Column(Integer, ForeignKey("members.id"), nullable=False, index=True)
